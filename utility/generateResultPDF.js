...
const generateResultsPDF1 = async (studentResult) => {
  return new Promise((resolve, reject) => {
    ...
    console.log('Generating PDF for:', studentResult); // Debugging statement

    doc.pipe(fs.createWriteStream(pdfPath));

    doc.fontSize(16).text(`Student Results`, { align: 'center' });

    doc.fontSize(12).text(`Academic Year: ${studentResult.academicYear}`);
    ...
    doc.on('finish', () => {
      console.log(`PDF generated at ${pdfPath}`); // Debugging statement
      resolve(pdfPath);
    });
